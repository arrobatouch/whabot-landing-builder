...
export function LandingAssistant({ onGenerateLanding, onManualMode, isGenerating = false }: LandingAssistantProps) {
  const handleBusinessInfoComplete = async (businessInfo: any) => {
    ...
    const blocks = [
      ...
      // 1.5⃣ Hero Simple - Usa misma estructura que hero-slide
      {
        id: 'hero-dynamic-1',
        type: 'hero',
        content: {
          title: landingData.heroTitle || 'Tu Negocio',
          subtitle: landingData.heroSubtitle || 'Líder en el sector',
          description: 'Descubri las mejores soluciones para tu empresa con calidad y confianza garantizadas.',
          backgroundImage: (landingData.heroImages && landingData.heroImages[0]) || 'https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=1920&h=1080&fit=crop',
      ...
      // 10 - Bloque de Características de Producto
      {
        id: 'product-features-1',
        type: 'product-features',
        content: {
          title: landingData.promoTitle || 'Promoción Especial',
          subtitle: 'Aprovechá nuestras ofertas',
          backgroundImage: landingData.sectionImages?.products?.[0] || '',
          features: [
            { icon: '🎉', title: 'Oferta Especial', description: 'Promoción por tiempo limitado' },
            { icon: '✨', title: 'Calidad Garantizada', description: 'Los mejores productos y servicios' },
            { icon: '🎯', title: 'Atención Personalizada', description: 'Servicio dedicado a cada cliente' }
          ],
          styles: {
            backgroundColor: 'bg-background',
            paddingY: 'py-16',
