...
export function ConversationalChat({ onBusinessInfoComplete, onManualMode, isGenerating = false }: ConversationalChatProps) {
    ...
    try {
      ...
      if (aiReply) {
        ...
        if (isLandingComplete) {
          ...
          console.log("🎯 CONVERSATIONAL CHAT: Landing completo detectado - MOSTRANDO APROBACIÓN (esperando clic del usuario)")
  ...
  const handleApproveLanding = () => {
    if (landingResult) {
      console.log("🎯 CONVERSATIONAL CHAT: Usuario APROBÓ el landing - Enviando datos...")
      ...
      // Parse the landing result and pass to parent SOLO cuando el usuario hace clic
      setTimeout(() => {
        console.log("📤 CONVERSATIONAL CHAT: Enviando landingContent a LandingAssistant...")
